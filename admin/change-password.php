...
if (isset($_POST['submit'])) {
    ...
    $query = "SELECT Password FROM faculty_login WHERE UserID = '" . $_SESSION['UserID'] . "'";
        ...
        if (password_verify($oldPassword, $row['Password'])) {
            if ($newPassword == $confirmPassword) {
                if (
                    ...
                    $errormsg = "Password must contain atleast one uppercase letter, one lowercase letter, one numeric digit, and one special character.";
                }
            } else {
                $errormsg = "New and Confirm Password do not Match!";
            }
        } elseif ($oldPassword == $row['Password']) {
            // Password is not hashed, but it matches the stored password
            if ($newPassword == $confirmPassword) {
                $hashedPassword = password_hash($newPassword, PASSWORD_DEFAULT);
                $updateQuery = "UPDATE faculty_login SET Password = '$hashedPassword' WHERE UserID = '" . $_SESSION['UserID'] . "'";
                mysqli_query($conn, $updateQuery);
...
<?php include("../includes/header.php"); ?>  
      ...
      <section id="main-content">
          <section class="wrapper">
          	<h3 style="padding-top:30px">Change Password</h3>
          	<div class="row mt">
          		<div class="col-lg-12">
                    ...
                    <?php }?>
                      ...
                      <form class="form-horizontal style-form" method="post" name="chngpwd" onSubmit="return valid();">
                          <div class="form-group">
                              <label style="padding-top: 20px;" class="col-sm-2 col-sm-2 control-label">Current Password</label>
